...
fn interpreter(mut file: File) {
    ...
    let mut array: [u8; CAPACITY] = [0; CAPACITY]; // memory
    ...
    let acceptable_symbols = [43, 44, 45, 46, 60, 62, 91, 93]; // filter everything that is not in the language
    ...
    loop {
        ...
        match token {
            ...
            b'+' => {
                array[current_memory_pointer] = array[current_memory_pointer] + 1;
                current_instruction += 1;
            }
            b'-' => {
                array[current_memory_pointer] = array[current_memory_pointer] - 1;
            ...
            b'[' => {
                if array[current_memory_pointer] != 0 {
            ...
            b'.' => {
                print!("{}", array[current_memory_pointer] as char); // it is buffered and will only flush when full
                current_instruction += 1;
            }
            ...
            b',' => {
                ...
                array[current_memory_pointer] = number;
                current_instruction += 1;
            }
            _ => {
                current_instruction += 1;
